...
public class CarSpecification {
    ...
    public static Specification<Car> hasNameLike(String name) {
        return (root, query, cb) -> name == null || name.isEmpty() ?
                cb.conjunction() :
                cb.like(cb.lower(root.join("carModel").get("name")), "%" + name.toLowerCase() + "%");
    }
    ...
    public static Specification<Car> isReady() {
        return (root, query, cb) -> cb.and(
                cb.equal(root.get("isReady"), true),
                cb.equal(root.get("deposited"), false),
                cb.equal(root.get("sold"), false)
        );
    }
