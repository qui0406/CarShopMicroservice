                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center justify-between mt-1">
                          <p className="text-sm text-gray-600 truncate mr-2">{c.lastMessage || "Chưa có tin nhắn"}</p>
                          <div className="flex items-center space-x-1 shrink-0">
                            <div className={`w-2 h-2 rounded-full ${getStatusColor(c.status)}`} />
                            <span className="text-[10px] text-gray-400 uppercase">{getStatusText(c.status)}</span>
                          </div>
                        </div>
                      </div>
                    </div>
        ...
        <div className="flex-1 flex flex-col h-full bg-white relative">
          {selectedConversation ? (
            <>
              ...
              <div ref={messageContainerRef} className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50 no-scrollbar">
                  ...
                  return (
                    <div key={key} className={`flex ${isMe ? "justify-end" : "justify-start"}`}>
                      <div className={`flex items-end space-x-2 max-w-[80%] ${isMe ? "flex-row-reverse space-x-reverse" : ""}`}>
                        {!isMe && (
                          <div className="w-8 h-8 shrink-0 bg-gradient-to-br from-green-400 to-blue-500 rounded-full flex items-center justify-center text-white text-sm font-medium overflow-hidden">
                            {msg.sender.avatar ? (
                        ...
                        <div className="flex flex-col space-y-1">
                          <div
                            className={`px-4 py-2 rounded-2xl ${
                              isMe
                                ? `bg-blue-600 text-white rounded-br-sm shadow-sm ${msg.isTemporary ? "opacity-70" : ""}`
                          ...
                          <div className={`text-[11px] font-medium text-gray-400 ${isMe ? "text-right" : "text-left"}`}>
                            {formatTime(msg.createdDate)}
                            {msg.isTemporary && <span className="ml-1 italic opacity-70">Đang gửi...</span>}
                          </div>
                        </div>
                      </div>
                    </div>
              ...
              <div className="bg-white border-t border-gray-200 p-4 shrink-0">
                <div className="flex items-end space-x-3 max-w-4xl mx-auto">
                  <button className="p-2 hover:bg-gray-100 rounded-full shrink-0"><Paperclip size={20} className="text-gray-500" /></button>
                  <div className="flex-1 relative">
                    <textarea
                      ...
                      className="w-full px-4 py-3 border border-gray-300 rounded-2xl focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none disabled:opacity-50 text-[14px]"
                      rows={1}
                      style={{ minHeight: 48, maxHeight: 120 }}
                    />
                  </div>
                  <button className="p-2 hover:bg-gray-100 rounded-full shrink-0"><Smile size={20} className="text-gray-500" /></button>
                  <button
                    onClick={handleSendMessage}
                    disabled={!message.trim() || sendingMessage || !selectedConversation?.id}
                    className="bg-blue-600 hover:bg-blue-700 disabled:bg-gray-300 disabled:cursor-not-allowed text-white p-3 rounded-full transition-colors shrink-0 shadow-sm"
                  >
                    <Send size={18} />
                  </button>
                </div>
            ...
            <div className="flex-1 flex items-center justify-center bg-gray-50">
              <div className="text-center">
                ...
                <h3 className="text-[18px] font-bold text-gray-900 mb-2">Hỗ trợ khách hàng</h3>
                <p className="text-sm text-gray-500 max-w-sm mx-auto">Chọn một cuộc trò chuyện từ danh sách bên trái để bắt đầu nhắn tin và giải đáp thắc mắc cho khách hàng.</p>
              </div>
            </div>
          )}
        </div>
